# Task 6.5: Session Manager

## Objective

Implement the main state machine that orchestrates BITalino data collection, network communication, and offline buffering.

## Dependencies

- Task 6.2 (BITalino Client) completed
- Task 6.3 (Convex Client) completed
- Task 6.4 (Offline Buffer) completed
...
- [ ] States: IDLE, CONNECTING, ACQUIRING, ENDING, ERROR
...
- [ ] Start BITalino when session found
- [ ] Stream data every batch interval
- [ ] Buffer data if network fails
...
- [ ] Include active session ID when applicable

### Error Recovery

- [ ] Reconnect BITalino on disconnect
- [ ] Retry network operations
- [ ] Mark session failed after max retries
...
```python
# raspberry-pi/src/session_manager.py
...
class SessionManager:
    ...
    async def _tick(self) -> None:
        ...
        # Sync buffered data if online and idle
...
1. Run unit tests with mocked components
2. Test with real BITalino and server:
   - Start manager
   - Create session in web UI
   - Verify data flows
   - End session
3. Test offline:
   - Disconnect network during session
   - Verify data buffered
